...
When working on local development with custom domains (like `*.polyplate.dev` and `polyplate.dev`), browsers normally
...
- Using `*.polyplate.dev` and `polyplate.dev` domains locally ensures consistency with staging and production environments.
...
- `_.polyplate.dev+1.pem` → the certificate
- `_.polyplate.dev+1-key.pem` → the private key

For clarity and consistency, rename these files
- `polyplate.pem` → certificate
- `polyplate-key.pem` → private key
...
- Configure your dev server to use `polyplate.pem` and `polyplate-key.pem` for HTTPS.
